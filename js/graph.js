...
//============================================================================================================================================================
    ...
    window.onload = function() {
        document.getElementById("filter-amazon").onclick = function() {
                updatebar("amazon");
                updateWordcloud("amazon");
                updateMap("amazon");           
         // Do your stuff here
        };
        ...
        document.getElementById("filter-google").onclick = function() {
            updatebar("google");
            updateWordcloud("google");
            updateMap("google");
         // Do your stuff here
        };
        document.getElementById("filter-netflix").onclick = function() {
            updatebar("netflix");
            updateWordcloud("netflix");
            updateMap("netflix");
         // Do your stuff here
        };
...
function updateMap(company){
    ...
    create_map(filtered_data, company,"#map")
    document.getElementById("map-title").innerHTML = "Polarity in US States ("+company+")";
 }
...
function redrawDonut(data){
     ...
     document.getElementById("half-donut-amazon").innerHTML = "";
     document.getElementById("half-donut-netflix").innerHTML = "";
     document.getElementById("half-donut-google").innerHTML = "";
...
d3.csv('data/employee_reviews_processed.csv', function(error, data) {
    ...
    create_map(filtered_data, 'microsoft',"#map")
...
function drawHalfDonut(modified_data)
{
    ...
    comment_amazon = allcomment[0]
    comment_apple = allcomment[1]
    comment_facebook = allcomment[2]
    comment_google = allcomment[3]
    comment_microsoft = allcomment[4]
    comment_netflix = allcomment[5]
    


    positive_amazon = modified_data['amazon']['positive_count'] 
    negative_amazon = modified_data['amazon']['negative_count'] 
    neutral_amazon = modified_data['amazon']['neutral_count'] 
    ...
    positive_google = modified_data['google']['positive_count'] 
    negative_google = modified_data['google']['negative_count'] 
    neutral_google = modified_data['google']['neutral_count'] 
    ...
    positive_netflix = modified_data['netflix']['positive_count'] 
    negative_netflix = modified_data['netflix']['negative_count'] 
    neutral_netflix = modified_data['netflix']['neutral_count'] 



    sum_amazon = positive_amazon+negative_amazon+neutral_amazon
    ...
    sum_google = positive_google+negative_google+neutral_google
    sum_apple = positive_apple+negative_apple+neutral_apple
    sum_netflix = positive_netflix+negative_netflix+neutral_netflix


    

    var data_amazon = [
        {name: "Positive", val: positive_amazon/sum_amazon, count: positive_amazon},
        {name: "Neutral", val: neutral_amazon/sum_amazon, count: negative_amazon},
        {name: "Negetive", val: negative_amazon/sum_amazon, count: neutral_amazon}
    ];
    ...
    var data_google = [
        {name: "Positive", val: positive_google/sum_google, count: positive_google},
        {name: "Neutral", val: neutral_google/sum_google, count: neutral_google},
        {name: "Negetive", val: negative_google/sum_google, count: negative_google}
    ];
    ...
    var data_netflix = [
        {name: "Positive", val: positive_netflix/sum_netflix, count: positive_netflix},
        {name: "Neutral", val: neutral_netflix/sum_netflix, count: neutral_netflix},
        {name: "Negetive", val: negative_netflix/sum_netflix, count: negative_netflix}
    ];
...
//=========================================================================================================  amazon ===============================
    ...
    var vis4 = d3.select("#half-donut-amazon")
        .append("svg:svg")
        .data([data_amazon])
        .style("width", w )
        .style("height", h)
    ...
    arcs.append("svg:path")
        ...
        .on("mouseenter", function(d, i) {
            if(check == 0)
            {
               ...
               .duration(500)
               .attr("d", mousearc) ; 
               d3.select(".label.amazon").text((d.value*100).toFixed(1)+"%");
               d3.select(".label.amazon").attr("fill", textcolor[i]);
               updatebar("amazon")
            }
        ...
        .on("mouseleave", function(d) {
            if(check == 0)
            {
                d3.select(this).transition()            
               .attr("d", arc);
               d3.select(".label.amazon").text((data_amazon[0].val*100).toFixed(1)+"%");
               d3.select(".label.amazon").attr("fill", textcolor[0]);
            ...
            if(d3.select(this).attr("stroke-width") == "0px")
            {
                if(check == 0)
                {
                    d3.select(this).attr("stroke-width","2px") 
                                ...
                                .attr("d", mousearc)  ; 
                    document.getElementById("comments").style.display = 'block';
                    document.getElementById("comment_text").innerHTML = comment_amazon[i];
                    document.getElementById("comment_title").innerHTML = 'Amazon Top-5 ' + d.data.name + ' Comments';
                    check = 1;
                }
    ...
    vis4.append("svg:text")
        .attr("transform", "translate(" + (r + 30) + "," + (r-30) + ")")
        .text( (data_amazon[0].val*100).toFixed(1)+"%")
        .attr("dy", "3rem")
        .attr("fill", textcolor[0]) 
        .attr("class", "label amazon")
        .attr("text-anchor", "middle")
...
//=========================================================================================================  google  ===============================
    ...
    var vis5 = d3.select("#half-donut-google")
        .append("svg:svg")
        .data([data_google])
        .style("width", w )
        .style("height", h)
    ...
    arcs.append("svg:path")
        ...
        .on("mouseenter", function(d, i) {
            if(check == 0)
            {
               ...
               .duration(500)
               .attr("d", mousearc) ; 
               d3.select(".label.google").text((d.value*100).toFixed(1)+"%");
               d3.select(".label.google").attr("fill", textcolor[i]);
               updatebar("google")
            }
        ...
        .on("mouseleave", function(d) {
            if(check == 0)
            {
                d3.select(this).transition()            
               .attr("d", arc);
               d3.select(".label.google").text((data_google[0].val*100).toFixed(1)+"%");
               d3.select(".label.google").attr("fill", textcolor[0]);
            ...
            if(d3.select(this).attr("stroke-width") == "0px")
            {
                if(check == 0)
                {
                    d3.select(this).attr("stroke-width","2px") 
                                ...
                                .attr("d", mousearc)  ; 
                    document.getElementById("comments").style.display = 'block';
                    document.getElementById("comment_text").innerHTML = comment_google[i];
                    document.getElementById("comment_title").innerHTML = 'Google Top-5 ' + d.data.name + ' Comments';
                    check = 1;
                }
    ...
    vis5.append("svg:text")
        .attr("transform", "translate(" + (r + 30) + "," + (r-30) + ")")
        .text( (data_google[0].val*100).toFixed(1)+"%")
        .attr("dy", "3rem")
        .attr("fill", textcolor[0]) 
        .attr("class", "label google")
        .attr("text-anchor", "middle")
...
//=========================================================================================================  netflix  ===============================
    ...
    var vis6 = d3.select("#half-donut-netflix")
        .append("svg:svg")
        .data([data_netflix])
        .style("width", w )
        .style("height", h)
    ...
    arcs.append("svg:path")
        ...
        .on("mouseenter", function(d, i) {
            if(check == 0)
            {
               ...
               .duration(500)
               .attr("d", mousearc) ; 
               d3.select(".label.netflix").text((d.value*100).toFixed(1)+"%");
               d3.select(".label.netflix").attr("fill", textcolor[i]);
               updatebar("netflix")
            }
        ...
        .on("mouseleave", function(d) {
            if(check == 0)
            {
                d3.select(this).transition()            
               .attr("d", arc);
               d3.select(".label.netflix").text((data_netflix[0].val*100).toFixed(1)+"%");
               d3.select(".label.netflix").attr("fill", textcolor[0]);
            ...
            if(d3.select(this).attr("stroke-width") == "0px")
            {
                if(check == 0)
                {
                    d3.select(this).attr("stroke-width","2px") 
                                ...
                                .attr("d", mousearc)  ; 
                    document.getElementById("comments").style.display = 'block';
                    document.getElementById("comment_text").innerHTML = comment_netflix[i];
                    document.getElementById("comment_title").innerHTML = 'Netflix Top-5 ' + d.data.name + ' Comments';
                    check = 1;
                }
    ...
    vis6.append("svg:text")
        .attr("transform", "translate(" + (r + 30) + "," + (r-30) + ")")
        .text( (data_netflix[0].val*100).toFixed(1)+"%")
        .attr("dy", "3rem")
        .attr("fill", textcolor[0]) 
        .attr("class", "label netflix")
        .attr("text-anchor", "middle")
...
function tooltipMap(n, d){	/* function to create html content string in tooltip div. */
	return "<h4>"+n+"</h4><table>"+
		"<tr><td>Positive</td><td>"+(d.pos)+"</td></tr>"+
		"<tr><td>Neutral</td><td>"+(d.neu)+"</td></tr>"+
		"<tr><td>Negative</td><td>"+(d.neg)+"</td></tr>"+
		"<tr><td>Polarity</td><td>"+(d.pol.toFixed(2))+"</td></tr>"+
		"</table>";
}


function create_map(data, company, svg_id){
	var cur_company_data = data.filter(function (d) {
		return d.company == company;
	});
	
	var mapData ={};
	['AK', 'AL', 'AR', 'AZ', 'CA', 'CO', 'CT', 'DC', 'DE', 'FL', 'GA', 'HI', 'IA', 'ID', 'IL', 'IN', 'KS', 'KY', 'LA', 'MA', 'MD', 'ME', 'MI', 'MN', 'MO', 'MS', 'MT', 'NC', 'ND', 'NE', 'NH', 'NJ', 'NM', 'NV', 'NY', 'OH', 'OK', 'OR', 'PA', 'PR', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VA', 'WA', 'WI', 'WV', 'WY', 'VT']
		.forEach(function(d){ 
			var state_data = cur_company_data.filter(function (el) {
			  return el.us_state == d;
			});
			if (state_data.length != 0){
				var find_counts = counts_and_most_helpful_texts(state_data)[company];
				var positive_count = find_counts['positive_count'];
				var negative_count = find_counts['negative_count'];
				var neutral_count = find_counts['neutral_count'];
				var polarity_avg = state_data.map(a => a['summarypolarity']);
				var sum = 0;
				// dividing by 0 will return Infinity
				// arr must contain at least 1 element to use reduce
				if (polarity_avg.length)
				{
					sum = polarity_avg.reduce(function(a, b) { return Number(a) + Number(b); });
					polarity_avg = sum / polarity_avg.length;
				}
				else{
					polarity_avg = 0;
				}
				...
				var color_interpolate = d3.scaleLinear()
					.domain([-1, 0, 1])
					.range(['Red','Gainsboro', 'Green']);
					
				mapData[d]={pos:positive_count, neu:neutral_count, 
						neg:negative_count, pol:polarity_avg, color:color_interpolate(polarity_avg)}; 
				
				continuous("#legend1", color_interpolate);
			}
			else{
				mapData[d]={pos:0, neu:0, 
						neg:0, pol:0, color:"Gainsboro"}; 
			}
		});
	uStates.draw(svg_id, mapData, tooltipMap);
...
function continuous(selector_id, colorscale) {
  ...
  var legendscale = d3.scaleLinear()
    .range([legendheight - margin.top - margin.bottom, (legendheight - margin.top - margin.bottom + 1)/2 , 1])
...
function get_average_ratings(data){
    var companies = ['amazon', 'apple', 'facebook', 'google', 'microsoft', 'netflix'];
    var col_names = ['overall-ratings', 'work-balance-stars', 'culture-values-stars', 'carrer-opportunities-stars', 'comp-benefit-stars', 'senior-mangemnet-stars'];
    var custom_col_names = ['Overall', 'Work-balance', 'Culture-values', 'Carrer-opportunities', 'Comp-benefit', 'Senior-mangemnet'];
    var mean_col_names = ['mean-overall-ratings', 'mean-work-balance-stars', 'mean-culture-values-stars', 'mean-carrer-opportunities-stars', 'mean-comp-benefit-stars', 'mean-senior-mangemnet-stars'];
    var result = {};
        ...
        if( (document.getElementById('formerEmp').checked) &&  (document.getElementById('currentEmp').checked)){
            ...
            filtered_data = load_data.filter(function(d){
             ...
             filtered_data = load_data.filter(function(d){
                ...
                redrawDonut(modified_data);
                updatebubble(filtered_data);
        }else alert("Pease Select Atleast ONE Employee Type!!!")
